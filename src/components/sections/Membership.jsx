...
function Membership() {
  return (
    <section>
        <Container>
            <Headline className="lg:text-center">Membership options</Headline>
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:mt-16">
                {membershipCardsData.map((card, index)=> (
                    <div key={index} className={`p-6 rounded-lg border border-neutral-200 space-y-8 relative overflow-hidden ${index !== 1 ? 'bg-primary-neutral-000' : 'bg-primary-neutral-100'}`}>
                        {index === 1 && <img src={images.pattern_glow} className="absolute z-0 top-[40%] left-[30   %] scale-130" alt="Glow" />}
                        ...
                        <Button isFull={true}>{card.cta_text}</Button>
                    </div>
                ))}
            </div>
        </Container>
